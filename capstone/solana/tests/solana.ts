...
describe("FarmLink", () => {
	...
	it("setup", async () => {
		try {
			...
			console.log(
				"Balances:\nfarmer: ",
				(await connection.getBalance(farmer.publicKey)) /
					LAMPORTS_PER_SOL,
				" \nconsumer: ",
				(await connection.getBalance(consumer.publicKey)) /
					LAMPORTS_PER_SOL,
				"\npayer: ",
			...
			farmLink = PublicKey.findProgramAddressSync(
				[
					Buffer.from("farmlink", "utf-8"),
					Buffer.from(farmName, "utf-8"),
				],
				program.programId
			)[0];
			...
			const consumerBalance = await connection.getBalance(
				consumer.publicKey
			);
			console.log(
	...
	it("create_product", async () => {
		const accounts = {
			...
			associatedTokenProgram:
				accountsPublicKeys["associated_token_program"],
			systemProgram: accountsPublicKeys["system_program"],
			tokenProgram: accountsPublicKeys["token_program"],
		};
